...
def test_acc(attack_net, target_net, dtype='uint8'):
    ...
    with torch.no_grad():
        ...
        correct_att += (predicted_att == labels).sum()
        if target_net is not None:
            outputs_tar = target_net(inputs * max_pixel)
            _, predicted_tar = torch.max(outputs_tar.data, 1)
            correct_tar += (predicted_tar.cuda() == labels).sum()

        print('Accuracy of the attack_net: %.2f %%' %
                (100. * correct_att.float() / total))
        if target_net is not None:
            print('Accuracy of the target_net: %.2f %%' %
...
def test_adver(net, tar_net, attack, dtype='uint8',white_box=False, clip_min=0.0, clip_max=1.0, target=False, nb_batch=10):
    ...
    if target:
        ...
        L2_distance = (adv_inputs_ori.cuda() - inputs.cuda()).squeeze()
        L2_distance = (torch.linalg.norm(torch.flatten(L2_distance, start_dim=1), dim=1)).data
        L2_distance = L2_distance * acc_sign
        total_L2_distance += L2_distance.sum()
        with torch.no_grad():
...
if opt.model == 'bird':
    ...
    # labelmap = np.loadtxt("./tflite_model/birds_labelmap.csv",delimiter=",",usecols=(0))
    # labelmap = torch.from_numpy(labelmap).to(device)
    ...
    # TfliteToOnnx('./tflite_model/', 'bird.tflite')                             
    # onnx_model = onnx.load('./out_model/bird.onnx')
    # onnx_modifier(onnx_model)
    # attack_net = ConvertModel(onnx_model, experimental=False).to(device).eval()
    ...
    kwargs.update(white_box=opt.white_box)
    kwargs.update(target=opt.target)
    # labelmap = np.loadtxt("./tflite_model/insects_labelmap.csv",delimiter=",",usecols=(0))
    # labelmap = torch.from_numpy(labelmap).to(device)
    ...
    kwargs.update(white_box=opt.white_box)
    kwargs.update(target=opt.target)
    # labelmap = np.loadtxt("./tflite_model/plant_labelmap.csv",delimiter=",",usecols=(0))
    # labelmap = torch.from_numpy(labelmap).to(device)
    ...
    kwargs.update(white_box=opt.white_box)
    kwargs.update(target=opt.target)
    # labelmap = np.loadtxt("./tflite_model/plant_disease_labelmap.csv",delimiter=",",usecols=(0))
    # labelmap = torch.from_numpy(labelmap).to(device)
    # labelmap = None
    ...
    kwargs.update(white_box=opt.white_box)
    kwargs.update(target=opt.target)
    # labelmap=np.loadtxt("./tflite_model/cassava_labelmap.csv",delimiter=",",usecols=(0))
    # labelmap=None
    ...
    kwargs.update(white_box=opt.white_box)
    kwargs.update(target=opt.target)
    # labelmap = torch.tensor(list(range(1, 4))).to(device)
    ...
    kwargs.update(target=opt.target)
    # labelmap=np.loadtxt("./tflite_model/cassava_labelmap.csv",delimiter=",",usecols=(0))
    # labelmap = torch.tensor(list(range(1, 1001))).to(device)
    # labelmap=None
